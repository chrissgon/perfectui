...
| `DESIGN-SYSTEM.md`              | Every component resolved to pixels and hex, for rebuilding the library as a design file. Generated by `bun run design-system`, which measures the real components in a browser rather than transcribing the stylesheet, and then runs `scripts/design-system-check.mjs`: 49 assertions covering every number the generator writes as prose — the chevron, the modal's limits, the timeline's rule, the contrast ratios — because a hand-written number is the one that drifts |
