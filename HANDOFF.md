...
Dropped by the maintainer's decision: `accordion-aligned`. `accordion-accented` came back later as `pui-highlighted` (decision 34).

Two more found afterwards. The accordion looked, in the maintainer's words, like nothing but a thin grey line. That was read as a missing header band and the `<summary>` was given the same band as `pui-card-header` — the wrong fix: the complaint was about the focus ring's color, and the maintainer asked for the band to be undone. The summary carries no background of its own today. Two consequences: the panel kept the top padding it gained, and `--pui-bg-emphasis` was left with no user in the library. The maintainer's answer to that was to keep it — it is the third level for an author who needs to lift something inside an already muted surface, such as a card — and to bring v0's `accordion-accented` back as `pui-highlighted`, which uses `--pui-bg-muted`.
...
- `tests/manual/index.html` lists the review pages; open that one rather than hunting for a file. The fixtures under `tests/fixtures/` now use relative paths too, so opening one by mistake still renders instead of showing unstyled markup.
- `tests/manual/overlays.html`: the parts that only exist once you interact with them — the four dropdown and tooltip directions plus a flip shown on purpose, the tooltip styles, stacking versus swapping modals in both recipes, the static backdrop, long content, and the accordion icon hook. Every case was driven through Playwright before the page was committed.
...
| `design-system/DESIGN-SYSTEM.md` | Every component resolved to pixels and hex, for rebuilding the library as a design file. Generated by `bun run design-system`, which measures the real components in a browser rather than transcribing the stylesheet, and then runs `scripts/design-system-check.mjs`: 49 assertions covering every number the generator writes as prose — the chevron, the modal's limits, the timeline's rule, the contrast ratios — because a hand-written number is the one that drifts |
